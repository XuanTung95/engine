...
public class ImageReaderPlatformViewManager {
  ...
  public void updateFrameInfo(
    ...
    frameManager.updateFrameInfo(
        viewId, rasterStart, width, height, top, left, haveOverlay, mutatorsStack);
  }
...
class ImageProducer {
  ...
  public synchronized FlutterOverlaySurface createOrResizeImageReader(
    ...
    return new FlutterOverlaySurface(
        imageReaderItem.imageReaderId, imageReaderItem.imageReader.getSurface());
  }
  ...
  private void onImageCreated(
      int viewId,
      ImageReader imageReader,
      ImageReaderData.ImageReaderItem imageReaderItem,
      Image image) {
    ...
    frameManager.onBitmapCreated(
        viewId,
        imageTimestamp,
        bitmap,
        new ReleaseImageCallback() {
          @Override
...
class FrameManager {
  ...
  private final Runnable onViewManagerAvailable =
      new Runnable() {
        @Override
        public void run() {
          processNextFrame();
  ...
  static class FrameInfo {
    ...
    public void setPlatformViewInfo(
        int viewId,
        int width,
        int height,
        int top,
    ...
    public void onBitmapCreated(
        int viewId, Bitmap bitmap, ImageProducer.ReleaseImageCallback releaseCallback) {
      for (PlatformViewInfo view : views) {
        if (view.getViewId() == viewId) {
  ...
  synchronized void updateFrameInfo(
      int viewId,
      long rasterStart,
      int width,
      int height,
  ...
  synchronized void onBitmapCreated(
      int viewId,
      long timestamp,
      Bitmap bitmap,
      ImageProducer.ReleaseImageCallback releaseCallback) {
    FrameInfo frame = null;
    for (FrameInfo item : frames) {
...
class ImageReaderData {
  ...
  public static class ImageReaderItem {
    ...
    void closeImage(Image image) {
      if (maxImageCount >= ImageProducer.KEEP_OPEN_IMAGES
          || openImages.size() >= ImageProducer.MAX_IMAGES - 1) {
