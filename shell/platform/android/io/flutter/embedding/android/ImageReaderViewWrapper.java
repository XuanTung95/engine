...
public class ImageReaderViewWrapper extends FrameLayout {
    ...
    public ImageReaderViewWrapper(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
...
//        });
    ...
    void setPlatformView(FlutterMutatorView view) {
        ...
        Log.e("Flutter", "setPlatformView MutatorsStack " + (info == null ? null : info.getMutatorsStack()));
        addView(view, 0);
    }
    ...
    void updatePlatformViewInfo(FrameManager.PlatformViewInfo info) {
        if (platformView != null) {
            platformView.readyToDisplay(info.getMutatorsStack(), info.getLeft(), info.getTop(), info.getWidth(), info.getHeight());
            platformView.invalidate();
        }
    }

    FrameLayout.LayoutParams _getLayoutParam(FrameManager.PlatformViewInfo info) {
        FrameLayout.LayoutParams layoutParam = new FrameLayout.LayoutParams(info.getWidth(), info.getHeight());
